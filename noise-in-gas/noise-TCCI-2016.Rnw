...
feedback on the quality of possible solutions to work correctly. This value, usually called {\em cost} or
...
a single floating point number as feedback; since these methodologies
...
needs to be either a `Yes', or `No', or `Not decidable' in most
...
\item In coevolutionary algorithms \cite{paredis1995coevolutionary,EvoStar2014:CoEvolutionary,DBLP:journals/soco/Parras-GutierrezARJ12},
...
algorithm optimizes the parameters of a hand-coded FSM that indicates how many ships are sent from each
...
infinite. Kurtosis is $6/\kappa$, implying the same. 
