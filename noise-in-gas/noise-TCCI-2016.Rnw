...
StarCraft has become a {\em de facto} testbed for AI research in complex RTS \cite{OntanonSURCP13}. In fact, given the complexity aspects of this successful game, researchers can deal with a high variety of topics, such as micro and macro management of units, temporal and spatial reasoning or uncertainty, among others.

The individuals described in this subsection have been generated using StarCraftGP \cite{Garcia15StarcraftGP}, a Genetic Programming (GP) framework that automatically generates the source code of high-level strategies of bots. As in previous examples, fitness of the individuals are generated confronting bots against different enemies, each one with a different strategy. In this case, each individual is confronted against 3 tiers of enemies (weak, medium and strong), each tier with 4 enemies. The fitness gives more weight to the stronger ones. In order to save execution time, at least one enemy of lower tiers must be defeated in order to confront the next tier, as it is predictable that if a bot do not win against weaker bots also it will not beat the stronger ones. Thus, fitness used in work \cite{Garcia15StarcraftGP} was used as hierarchical fitness, that is, one victory in a stronger tier is considered better than two in a lower one. However, for this work, we have calculated an aggregated fitness that respect this decision giving different weights to each tier. The next equation describes the fitness:
\begin{equation}
F_{StarCraft}=21\times A + 5\times B + C + R
\end{equation}
Where $A$ is the number of victories against the enemies of the strongest tier, $B$ is the number of victories against the medium tier and $C$ is the number of victories against the weakest enemies. Also, a coefficient of the aggregated score at the end of all the games has been added to deal with ties in number of victories.
