...
statistical distribution are, in general, away from normal
...
the set of fitness measures could look like $x x x y x x x$. As
...
performed for some new selection operators, the best way to model uncertainty
