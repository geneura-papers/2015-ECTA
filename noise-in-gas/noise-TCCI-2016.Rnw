...
\sloppypar The most  comprehensive, although no longer recent, review of the state of the art in 
...
multiobjective optmization was published by Goh and Tan \cite{goh2007investigation}. In that first survey  
...
amount of error. 
%PABLO: I do not know when to use the semicolons in pharses, but I think the previous one is quite large
They suggest several methods, based either on using
...
In fact, it has been recently proved that using {\em sex}, that is, crossover, %PABLO: Well, the "sex" thing is kinda... funny :P I suggest removing it, just in case
...
other measures such as the median which might be more adequate for 
...
the results of these two strategies and which one, if any, is the best
...
generation of character backstories in artificial worlds, described in subsection \ref{ss:made}, optimization of bots for playing the real time strategy game (RTS) Planet Wars in \ref{ss:pw}, optimization of the ghost team in Ms. Pac-Man, which will be described in subsection \ref{ss:pacman}, and automatic generation of autonomous players for the famous RTS StarCraft, explained in subsection \ref{ss:starcraft}. These four problems have been chosen for several reasons, the most important of which is that we have been working on them and thus have data %[Pedro] should the word "we" appear before "have" ?
% Why not? - JJ
