...
functions; then, different options for dealing with it are
...
such as the $\mu$+1 EA [REFERENCE NEEDED], or evolutionary programming, 
...
there is no big decrease in efficiency as long as oversized
...
Another way to deal with uncertainty,
more theoretically sound, is using {\em real} averaging: 
...
libraries such as ECJ [REFERENCE OR LINK NEEDED].
...
individual memory % Alberto - what is "an individual memory"?
