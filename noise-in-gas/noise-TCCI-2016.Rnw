...
Ms. Pac-Man [REFERENCE NEEDED] is a variant of the famous Pac-Man game [REFERENCE NEEDED] that extends its
...
the Ms. Pac-Man vs Ghosts competition [REFERENCE NEEDED], where participants can submit
...
The individuals described in this subsection have been generated using StarCraftGP \cite{Garcia15StarcraftGP}, a Genetic Programming (GP) [REFERENCE NEEDED, Koza1994?] framework that automatically generates the source code of high-level strategies of bots. As in previous problems, the fitness of one individual is computed pitting the bot against different enemies, each one following a different strategy. 
...
Conversely, to ease comparisions among noise in the present study, we have calculated an aggregated fitness function that still respects this decision, that is, priorizing victories of harder tiers, 
...
and measuring
their fitness 100 times, intentionally using much more repetitions than a normal optimizationg method would. Thus, every individual is represented by a
random variable with the 100 measures taken with its
fitness. % Alberto - can we replace "with the 100 measures taken with its fitness" with "sampled 100 times"? Would it make sense?
...
hypothesis, we plotted the distribution's {\em skewness}, that is, its asymmetry, and its {\em kurtosis}, which is a parameter related to its shape. 
...
related to their $\alpha$ and $\kappa$ parameters, for instance.
...
generation. % Alberto - ok, but why? Why didn't you take 50 individuals in every generation?
...
one. % Alberto - I am not sure I understood this phrase...
...
had one or a few. % Alberto - I could not understand the last phrase ^_^;
...
distribution followed by the stochastic fitness values of single individuals 
in several
case studies in the area of games.
...
not a fixed number but a random variable. We
...
as Cauchy, which have been used repeatedly in the literature; but 
...
computed and plotted for candidate solutions from each one of the case studies, 
proving that not
...
even possess a reliable average. A better way of comparing any fitness with
...
distribution with varying shape and symmetry such as Beta. However, in this case we should take into account that ``true''
...
If the fitness evaluation is expensive and tests want to be
