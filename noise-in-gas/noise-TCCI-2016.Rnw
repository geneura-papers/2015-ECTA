...
\title{Signal in noise: a study of uncertainty in the context of
  evolutionary optimization in games}
...
% Antonio - I rewrite the abstract (It tells the same but in other words)  
  It is usual in many optimization processes that the fitness or the considered measure of optimality for the potential solutions presents the so-called \textit{uncertainty} or more commonly \textit{noise}. It arises due to the nature of the function that represents this measure, which is a random variable.
This paper describes a study of these kind of variables, and more specifically, on the statistical distribution they show. We consider that the variables follow a normal distribution, so they can be modelled as a mathematical operation (adding or multiplication) over a common (non-noisy) fitness function.
The work analyzes four different problems, all of them enclosed in the videogames context where Evolutionary Computation methods have been applied, and where the evaluation of any individual is performed playing the game (which is a noisy environment itself).
% Antonio - check if this asseveration is correct
We demonstrate that the evaluations of the fitness functions do not follow any statistical model, being different not only from one problem to the next, but also in different phases of the optimization in a single problem.
...
%PABLO: I've divided this phrase, it was too long
...
The individuals described in this subsection have been generated using StarCraftGP \cite{Garcia15StarcraftGP}, a Genetic Programming (GP) framework that automatically generates the source code of high-level strategies of bots. As in previous problems, the fitness of one individuals is generated confronting the bot against different enemies, each one following a different strategy. 
