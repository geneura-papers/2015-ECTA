...
% Antonio - I rewrite the abstract (It tells the same but in other words)  
  ...
  values when measured repeatedly, due to the evaluation process or to
  ...
  individual is performed playing the game (which is a noisy
  environment itself). 
...
variable, not a single value or a vector of crisp values. 
...
The main problem we try to solve in this problem is the lack
of an exhaustive research of the behavior of fitness as a random variable.
...
tools. Our final objective is to eventually find a model that is as general
...
\cite{goh2007investigation}. Increasinly more papers are concerned
with this type of optimization, so this makes more interesting any
advance in this field. 

In his survey, Jin and Branke  state that uncertainty is categorized into noise,
...
In another hand, some authors \cite{Flores2011} propose using 
a new rank-based selection operator and a new rank-based mutation operator 
in order to evolve the neural network topology
used as a controller for a robot. Results show that those operators were 
...
and evolution of neural networks \cite{castilloGECCO99,merelo:ESNN}. 
...
The individuals described in this subsection have been generated using StarCraftGP \cite{Garcia15StarcraftGP}, a Genetic Programming (GP) framework that automatically generates the source code of high-level strategies of bots. As in previous problems, the fitness of one individuals is computed confronting the bot against different enemies, each one following a different strategy. 
...
% Antonio - are they 'different' as a have written?
The fitness function assigns a higher weight to the stronger ones, i.e. a hierarchical fitness was used in the original, so one victory in a stronger tier was considered better than four in the inmediately weaker one. 
...
However, for the present study, we have calculated an aggregated fitness function that respects this decision, that is, priorizing victories of higher tiers, 
