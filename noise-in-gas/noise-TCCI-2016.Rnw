...
The fitness function assigns a higher weight to the stronger ones, i.e. a hierarchical fitness has been used, so one victory in a stronger tier is considered better than two in a weaker one. 
% Antonio - Check my rephrasing and please try to clarify this giving a more complete explanation. Is the paragrpah below explaining this? ;)

However, for the present study, we have calculated an aggregated fitness function that respects this decision 
% Antonio - Which decision do you refer to?
giving different weights to each tier. The next equation describes the fitness:
\begin{equation}
F_{StarCraft}=21\times A + 5\times B + C + R
\end{equation}

Where $A$ is the number of victories against the enemies of the strongest tier, $B$ is the number of victories against the medium tier, and $C$ is the number of victories against the weakest enemies. Also, a coefficient of the aggregated score at the end of all the games, $R$ has been added, in order to deal with ties in number of victories.
% Antonio - Please, explain this better. ;)
...
% Antonio - with the current score, right?
