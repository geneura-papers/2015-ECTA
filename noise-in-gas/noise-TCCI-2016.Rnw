...
%this one - JJ
using the results obtained from the Genebot algorithm \cite{DBLP:conf/iwann/Fernandez-AresMMGF11}. This
algorithm optimizes seven parameters of a hand-coded FSM, namely:
\begin{itemize}
	\item the percentage of starships the bot sends (regarding the number of starships in the planet).
   \item the probability this happens.
	\item the weight of the number of starships and the planet growth rate.
   \item the distance from the base planet to the target planet. 
	\item the proportion and percentage of extra starships that the bot sends from the base planet to the target planet. 
	\item the probability of sending extra fleets from the colonies to the target planet.
\end{itemize}

These values are used in expressions used by the bot to take decisions, such as the selection of the target planet to attack or reinforce. 
A GA was applied to evolve (improve) these parameters off-line, i.e., not during the battles. It followed a generational scheme \cite{Michalewicz} with 
elitism (the best solution always survives). A Two-Tournament selection was performed, together with a BLX-alpha crossover 
\cite{Herrera03ataxonomy} ($\alpha$ = 0.5) and an operator which mutates the value of a random gene by adding or subtracting a random quantity in the $[0,1]$ interval. 

The generated bot is not deterministic, as some of the jumps of the states are based on probabilities. 
