...
simple Real-Time Strategy (RTS) game\footnote{actually the game is
  turn-based, but with just a few milliseconds to decide the actions},
in which the players control spaceships (units) and have to conquer
planets (structures/resources). These planets generate spaceships
(with a different rate depending on their size). The objective is to
defeat the enemy's fleet.  
