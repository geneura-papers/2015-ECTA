...
grouping the number of victories and the number of actions to win or
be defeated. Selection is performed using a binary tournament
comparing first the number of victories and comparing the rest of the
elements of the fitness (number of movements to win, and then number
of movements to lose) in case of equality in this elements. 
