...
Planet Wars \footnote{\url{http://planetwars.aichallenge.org/}} is a
...
the map to build units and structures. Computational intelligence
...
(planets). Spaceships are generated in the planets owned by the player
and they are used to conquer the enemy planets. In this paper we are
using the results obtained from the GPBot algorithm \cite{EvoStar2014:GPBot}. This
algorithm uses genetic programming (GP) to automatically generate a
decision tree for the player indicating how many ships send from each
planet to attack or reinforce another planet. The generated bot is not
deterministic, as some of the decisions of the nodes are based in
probabilities. %FERGU: DE DONDE SE SACAN LOS RESULTADOS? (PONER REF) 
%puesta ref. 

Fitness is calculated confronting 10 times the bot modelled by the GP tree against
a previous bot in the literature (Exp-Genebot)
\cite{DBLP:conf/cig/Fernandez-AresGMG12}. A hierarchical
fitness is used, grouping the number of victories and the number of
actions to win or be defeated. Selection is performed using a binary
tournament comparing first the number of victories and comparing the
rest of the elements of the fitness (number of actions to win, and
then number of actions to lose) in case of equality in this elements. 
