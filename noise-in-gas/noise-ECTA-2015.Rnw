...
Fitness is calculated confronting the bot modelled by the tree against a previous bot in the literature (Exp-Genebot) \cite{DBLP:conf/cig/Fernandez-AresGMG12} 10 times. A hierarchical fitness is used, grouping the number of victories and the number of actions to win or be defeated. Selection is performed using a binary tournament comparing first the number of victories and comparing the rest of the elements of the fitness (number of actions to win, and then number of actions to lose) in case of equality in this elements.
