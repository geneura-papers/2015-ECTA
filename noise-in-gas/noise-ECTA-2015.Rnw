...
% Antonio - referencia a esto (pos esto)
...
interesting of which is the possibility of applying different
strategies to the ghosts behavior; the original version included fixed
and different behaviors for each of the four ghosts. Another extension is the inclusion of a
random movement to prevent the use of patterns for clearing each round.
...
The method whose fitness has been analyzed in this paper follows the
...
The fitness in this method is computed by computing the maximum score
in 8 different matches. 
