...
In this paper we are using the results obtained from the so-called GeneBot algorithm \cite{}. This algorithm applies a Genetic Algorithm (GA) to improve the behavioral parameters and thresholds (of a Finite State Machine) that define how the bot behaves in a battle against any opponent.
