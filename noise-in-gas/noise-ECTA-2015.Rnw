...
\abstract{Noise or uncertainty appear in many optimization processes
  ...
  it can be modelled via an additive or multiplicative noise. 
% Antonio - se modela con ruido o con una función?
In this paper we will look at several uncertain optimization problems, addressed by means of Evolutionary Algorithms, 
% Antonio - añado una cosilla para aclarar que son evolutivos
 and prove that there is no single statistical model these variables
...
methods need a single floating point number; since they are based on comparisons,
...
cuestion ``Is this solution better than the other'' needs to be either a crisp
Yes or No of simply ``Impossible to know''.

In many cases, however, the fitness or cost of a solution cannot be
described by a crisp value. In cases where there is uncertainty in the
measure, that is, in most real world, physical cases, or in the procedure used to
...
optimization problems:\begin{itemize}
...
\item When training any kind of neural network, such as those in
  ...
  physical installation, introducing another kind of randomness. Since training a neural network is a
  stochastic procedure, the error rate obtained after every training
  run will also follow  a statistical distribution.
\item When evolving game bots \cite{bots:evostar}. In this case, the
  ...
  that final score will also be {\em uncertain} or {\em noisy}; in
  some cases, too, the bot itself will rely on probabilities to
...
In all these case it cannot be said that there is {\em noise} added to
...
games\footnote{One evaluation in some of those works consists, in
  fact, in an average of several evaluations (in different maps or
  considering different opponents, for instance)}
\cite{bots:evostar,NoisyFitness_JCST, liberatore:pacman}  
...
% Antonio - yo estoy lo pongo en duda, por lo menos en el caso de los juegos, aunque existe un factor de ruido, no se corresponde con algo aleatorio (o casi), a menos que los rivales estén muy igualados o que se produzcan situaciones MUY extrañas en el mapa. Normalmente los buenos ganarán a los malos y viceversa.
...
But real populations are finite, so the usual approach to dealing with fitness with a degree of randomness is to increase the population size to a value bigger than would be needed in a non-noisy environment. 
% Antonio - en juegos el tratamiento es la repetición de evaluaciones (creo recordar que hay algunos trabajos que lo indican así, aparte del nuestro de la revista JCST). :D    Vale, esto lo pone debajo... 
...
sample size used for averaging \cite{aizawa1994scheduling}. 
% Antonio - Decir que nosotros mismos lo hemos aplicado en juegos como he comentado arriba. Cita JCST. ;)
...
% Antonio - Sección describiendo los tres problemas considerados (cogerlas de los artículos anteriores).

The fitness of three different problems, all of them related to computational intelligence in games has been used in this paper: optimization of the ghost team in Ms. Pacman, which will be described in subsection \ref{ss:pacman}, generation of character backstories in artificial worlds \ref{ss:made} and optimization of bots for playing the real time strategy game planet wars \ref{ss:pw}. These three problems have been chosen for several reasons, the most important of which is that we have been working on them and thus have data available; another reason is that the origin of the uncertainty is different in the three cases. In the case of Ms. Pac-Man, it is due to the nature of the game; in the case of MADE, fitness is computed through a simulation; finally, in the case of Planet Wars, the bot themselves have a random component, with its representation including probabilities of different courses of action. It is not a complete representation of all causes of uncertainty, but the sample is big enough so that we can generalize the results obtained, which will be presented in the next section.
...
\ref{ss:made}
MADE (Massive Drama Engine for non-player characters) \cite{garcia14my} is a framework for the automatic generation of virtual worlds that allow the emergence of backstories for secondary characters in videogames. This framework is focused in search the generation of {\em archetypes} in the generated stories. An archetype is a well-known behaviour present in the imaginary collective (for example, a ``hero'' or a ``villain''). Given a fitness to model the existence of different $N_a$ archetypes for a virtual world, MADE uses a genetic algorithm to optimize the parameter values of a Finite State Machine (FSM) that model the agents of that world. For the evaluation, a world is simulated using this parameter set, and the log is analysed to detect behaviours of the world agents to match with the desired archetypes.

As the evolved parameters are the probabilities to jump from one state to another in the FSM, each fitness evaluation is performed executing the virtual world 5 times per individual, obtaining the average fitness. Selection is, therefore, performed comparing this average fitness, using a binary tournament in this case. Fitness values are comprised between 0 and $N_a$ and are calculated taking into account the rate of the archetypes in the execution log.


\subsection{Real time strategy: Planet Wars}
\ref{ss:pw}
Planet Wars \footnote{\url{http://planetwars.aichallenge.org/}} is a simple Real time strategy (RTS) game. RTS games are played in real time (that is, they are not turn-based), and the objective is to defeat the enemy using resources available in the map to build units and structures. Planet Wars is used in the computational intelligence in games area as it provides a simplification of the elements of the RTS: one kind of units (spaceships) and one kind of resources and structures (planets). Spaceships are generated in the planets owned by the player and they are used to conquer the enemy planets. In this paper we are using the results obtained from the GPBot algorithm \cite{}. This algorithm uses genetic programming (GP) to automatically generate a decision tree for the player indicating how many ships send from each planet to attack or reinforce another planet. The generated bot is not deterministic, as some of the decisions of the nodes are based in probabilities. %FERGU: DE DONDE SE SACAN LOS RESULTADOS? (PONER REF)

Fitness is calculated confronting the bot modelled by the tree against a previous bot in the literature (Exp-Genebot) \cite{ExpGenebot12} 10 times. A hierarchical fitness is used, grouping the number of victories and the number of actions to win or be defeated. Selection is performed using a binary tournament comparing first the number of victories and comparing the rest of the elements of the fitness (number of actions to win, and then number of actions to lose) in case of equality in this elements.

\section{Results}
\ref{sec:res}
% ******************************************************************************

% Antonio - Sección de experimentos y resultados.
% Podías haberla puesto directamente - JJ
% ******************************************************************************
